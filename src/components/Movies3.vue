...
start(){
  this.getCinemas().then(response => {
      response.cinemas.forEach((cinema, i) => {
          cinemas.push(this.objectTemplate(cinema.name, i))
          this.getFilms(cinema.id).then(response => {
              response.listings.forEach((film, j) => {
                  cinemas[i].films.push(film)
                  this.getPoster(cinemas[i].films[j].title).then(response => {
                      cinemas[i].films[j].img = imageUrl + response.results[0].poster_path
                  })
              })
          })
      })
